...
mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloConfigDB -e "INSERT INTO Namespace (AppId, ClusterName, NamespaceName, DataChange_CreatedBy, DataChange_LastModifiedBy) SELECT $APP_ID, \"$ENVIRONMENT\", \"$APP_HOST\", \"apollo\", \"apollo\" FROM DUAL WHERE NOT EXISTS (SELECT * FROM Namespace WHERE AppId=\"$APP_ID\" AND ClusterName=\"$ENVIRONMENT\" and NamespaceName=\"$APP_HOST\");"
...
mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "INSERT INTO Permission (PermissionType, TargetId, DataChange_CreatedBy, DataChange_LastModifiedBy) SELECT \"ModifyNamespace\", \"$APP_ID+$APP_HOST+DEVELOPMENT\", \"apollo\", \"apollo\" FROM DUAL WHERE NOT EXISTS (SELECT * FROM Permission WHERE TargetId=\"$APP_ID+$APP_HOST+DEVELOPMENT\" AND PermissionType=\"ModifyNamespace\");"
mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "INSERT INTO Permission (PermissionType, TargetId, DataChange_CreatedBy, DataChange_LastModifiedBy) SELECT \"ReleaseNamespace\", \"$APP_ID+$APP_HOST+DEVELOPMENT\", \"apollo\", \"apollo\" FROM DUAL WHERE NOT EXISTS (SELECT * FROM Permission WHERE TargetId=\"$APP_ID+$APP_HOST+DEVELOPMENT\" AND PermissionType=\"ReleaseNamespace\");"
...
mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "INSERT INTO Role (RoleName, DataChange_CreatedBy, DataChange_LastModifiedBy) SELECT \"ModifyNamespace+$APP_ID+$APP_HOST+DEVELOPMENT\", \"apollo\", \"apollo\" FROM DUAL WHERE NOT EXISTS (SELECT * FROM Role WHERE RoleName=\"ModifyNamespace+$APP_ID+$APP_HOST+DEVELOPMENT\");"
mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "INSERT INTO Role (RoleName, DataChange_CreatedBy, DataChange_LastModifiedBy) SELECT \"ReleaseNamespace+$APP_ID+$APP_HOST+DEVELOPMENT\", \"apollo\", \"apollo\" FROM DUAL WHERE NOT EXISTS (SELECT * FROM Role WHERE RoleName=\"ReleaseNamespace+$APP_ID+$APP_HOST+DEVELOPMENT\");"
...
mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloConfigDB -e "INSERT INTO Item (NamespaceId, \`Key\`, Value, LineNum, DataChange_CreatedBy, DataChange_LastModifiedBy) SELECT $appnamespaceid, \"username\", \"user\", 1, \"apollo\", \"apollo\" FROM DUAL WHERE NOT EXISTS (SELECT * FROM Item WHERE NamespaceId=\"$appnamespaceid\" AND \`Key\`=\"username\");"
...
modifyrole_env=`mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "select Id from Role where RoleName=\"ModifyNamespace+$APP_ID+$APP_HOST+DEVELOPMENT\";"`
modifyroleid_env=`echo $modifyrole_env | awk '{ print $2 }'`
modifyrolepermission_env=`mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "select Id from Permission where PermissionType=\"ModifyNamespace\" and TargetId=\"$APP_ID+$APP_HOST+DEVELOPMENT\";"`
...
releaserole_env=`mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "select Id from Role where RoleName=\"ReleaseNamespace+$APP_ID+$APP_HOST+DEVELOPMENT\";"`
releaseroleid_env=`echo $releaserole_env | awk '{ print $2 }'`
releaserolepermission_env=`mysql -uroot -p$PASSWORD -h 192.168.49.2 -P 30306 -D ApolloPortalDB -e "select Id from Permission where PermissionType=\"ReleaseNamespace\" and TargetId=\"$APP_ID+$APP_HOST+DEVELOPMENT\";"`
